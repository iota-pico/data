...
describe("Address", () => {
    describe("create", () => {
        it("can fail with undefined params", () => {
            chai.expect(() => Address.create(undefined)).to.throw("The address");
        });

        it("can fail with invalid length", () => {
            chai.expect(() => Address.create(Trytes.create("A"))).to.throw("characters in length");
        });
        ...
        it("can succeed with valid length with checksum", () => {
            const obj = Address.create(Trytes.create("A".repeat(81) + "B".repeat(9)));
            chai.expect(obj.toTrytes().toString()).to.equal("A".repeat(81));
